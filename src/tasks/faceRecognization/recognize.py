import sys
sys.path.append(r'D:\prj_python\backend')
import threading
import time
import cv2
import numpy as np
...
from utils import compare_encodings, read_features
...
def tracking_video(detector, args, video_path):
    """
    Face tracking from a video file.
    ...
    # Replace video source with a file path
    cap = cv2.VideoCapture(video_path)
    fps = cap.get(cv2.CAP_PROP_FPS)  # Lấy FPS từ video
    delay = int(1000 / fps)  # Chuyển đổi FPS thành thời gian chờ tính bằng ms
...
def main():
    ...
    file_name = configure.WeightConfigs.TRACKING_CONFIG
    config_tracking = load_config(file_name)
    video_path = r"D:\prj_python\backend\test_data\3aa3652a-a251-41b8-b38e-7345becf8966.mp4"
    thread_track = threading.Thread(
        target=tracking_video,
        args=(
            detector,
            config_tracking,
